...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description: 'Auditoría UX y rediseño de la app de banca móvil de Imaginbank — centrado en prevención de errores, claridad del estado del sistema, y confianza del usuario en operaciones financieras críticas.',
    longDescription: 'Un usuario abre su app bancaria para hacer una transferencia de 800€. Toca "Enviar". La pantalla se queda en blanco dos segundos. No hay spinner. No hay mensaje. **¿Se ha enviado? ¿Ha fallado? ¿Se ha enviado dos veces?**\n\nEste momento — dos segundos de silencio del sistema en una operación financiera — es exactamente el tipo de fallo que destruye la confianza en una app bancaria. No es un problema estético. Es un problema de sistema: estados no comunicados, errores sin contexto, y flujos que dejan al usuario solo en los momentos de mayor incertidumbre.\n\nEste proyecto es una auditoría UX rigurosa de Imaginbank (CaixaBank), evaluada con las 10 Heurísticas de Nielsen, que identificó **12+ fallos de usabilidad categorizados por severidad** y propuso soluciones centradas en tres principios: **claridad del estado del sistema, prevención de errores antes de que ocurran, y feedback inmediato en cada operación financiera.**',
    challenge: 'La app de Imaginbank presentaba fallos sistémicos que afectaban directamente a la confianza del usuario: **ausencia de feedback en operaciones financieras** (el usuario no sabía si una transferencia se había completado), **mensajes de error genéricos** ("Ha ocurrido un error" sin contexto ni solución), **nomenclatura inconsistente** (la misma acción se llamaba "Transferir", "Enviar" o "Bizum" según la pantalla), y **navegación rígida** sin opciones de deshacer. En banca móvil, cada uno de estos fallos no solo frustra — erosiona activamente la confianza en la plataforma que gestiona el dinero del usuario.',
    solution: 'Auditoría heurística completa (10 Heurísticas de Nielsen) combinada con testing cualitativo con usuarios de 18-25 años. Se diagnosticaron fallos categorizados por severidad y se propusieron soluciones sistémicas: **sistema de estados para cada operación** (loading, success, error, pending), **mensajes de error accionables** con causa y solución, **confirmaciones explícitas antes de acciones irreversibles**, y **nomenclatura unificada** en toda la app. Cada propuesta incluye el estado actual, el problema, la recomendación y el impacto esperado.',
    results: [
      '12+ fallos de usabilidad identificados y categorizados por severidad (crítico, alto, medio) con metodología Nielsen',
      'Sistema de estados propuesto para todas las operaciones financieras: loading, success, error, pending — eliminando la incertidumbre',
      'Reducción estimada del -35% en llamadas a soporte gracias a mensajes de error accionables y feedback visual',
      'Nomenclatura unificada: glosario UX con terminología consistente para eliminar la confusión entre secciones',
      'Propuesta de confirmación explícita pre-acción para todas las operaciones irreversibles (transferencias, pagos, cancelaciones)',
    ],
    stack: ['Figma', 'Chrome'],
    ...
    checks: [
      'Ausencia de feedback en operaciones financieras críticas: el usuario no sabe si una transferencia se ha completado, está en proceso o ha fallado',
      'Mensajes de error genéricos sin contexto ni solución — en banca, cada error no explicado erosiona la confianza en la plataforma',
      'Nomenclatura inconsistente y navegación rígida: la misma acción tiene nombres diferentes según la pantalla, sin opción de deshacer',
    ],
    caseSections: [
      // ── 01. El Reto ──
      {
